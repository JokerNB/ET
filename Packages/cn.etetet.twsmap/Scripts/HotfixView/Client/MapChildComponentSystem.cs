...
namespace ET.Client
{
    ...
    {
        [EntitySystem]
        private static void Awake(this ET.Client.MapChildComponent self, UnityEngine.Vector2 args2, UnityEngine.Sprite args3,
        UnityEngine.Transform args4, UnityEngine.GameObject args5, Vector2 spriteSize)
        {
            self.centerPos = args2;
            self.sprite = args3;
            self.spriteSize = spriteSize;
            self.Create(args4, args5);
        }

        public static void Create(this MapChildComponent self, Transform parent, GameObject go_Clone)
        {
            self.go = GameObject.Instantiate(go_Clone, parent, true);
            self.spriteRenderer = self.go.GetComponent<SpriteRenderer>();
            self.spriteRenderer.sprite = self.sprite;
            self.go.transform.localPosition = self.centerPos;
            if (!self.go.activeSelf)
                self.go.SetActive(true);
        }

        public static bool CheckNeedUpdate(this MapChildComponent self)
        {
            if (!self.go.activeSelf)
                return true;
        ...
        public static void UpdatePos(this MapChildComponent self, float x, float y)
        {
            self.centerPos.x = x;
            self.centerPos.y = y;
            self.go.transform.localPosition = self.centerPos;
