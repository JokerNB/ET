...
namespace ET.Client
{
    ...
    public static partial class FUIComponentSystem
    {
        public static async ETTask<FUIEntity> ShowPanelAsync<T>(this FUIComponent self) where T: Entity, IAwake, new()
        {
            ...
            FUIEntity fuiEntity = await self.InnerShowPanelAsync<T>(panelInfo);
        ...
        /// </summary>
        private static async ETTask<FUIEntity> InnerShowPanelAsync<T>(this FUIComponent self, PanelInfo panelInfo) where T: Entity, IAwake, new()
        {
            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.ShowingPanels, (int)panelInfo.PanelId, 3000))
            {
                var fuiEntity = self.GetFirstFUIEntityByPanelId(panelInfo);
                if (fuiEntity != null)
                {
                ...
                fuiEntity = await self.CreatePanelAsync<T>(panelInfo);
                return fuiEntity;
            }
        }
